...
function HgAdapter:file_history_options(range, paths, argo)
  ...
  if log_options.L and next(log_options.L) then
    log_options.follow = true -- '--follow' if required with '-L'
  end
...
function HgAdapter:prepare_fh_options(log_options, single_file)
  ...
  return {
    ...
    flags = utils.vec_join(
      line_trace,
      (o.follow and single_file or (#log_options.L > 0)) and { "--follow" } or nil,
...
function HgAdapter:parse_fh_data(state)
  ...
  for i = 1, #cur.numstat - 1 do
    ...
    local stats = {}
    local name, changes, diffstats = cur.numstat[i]:match("(.*)|%s+(%d+)%s*([+-]*)")
    name = vim.trim(name)
...
HgAdapter.show = async.wrap(function(self, path, rev, callback)
  ...
  local job = Job:new({
    ...
    on_exit = async.void(function(j)
      ...
      if j.code ~= 0 then
        local out = table.concat(j:stderr_result())
